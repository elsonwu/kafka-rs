...
<head>
    <title>Code coverage report for confluent-kafka-javascript/lib/kafkajs/_consumer.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../../index.html">All files</a> / <a href="index.html">confluent-kafka-javascript/lib/kafkajs</a> _consumer.js</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">61.63% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>461/748</span>
            </div>
            ...
            <div class='fl pad1y space-right2'>
                ...
                <span class='fraction'>162/365</span>
            </div>
            ...
            <div class='fl pad1y space-right2'>
                <span class="strong">61.8% </span>
                <span class="quiet">Lines</span>
                <span class='fraction'>458/741</span>
            </div>
...
<a name='L2076'></a><a href='#L2076'>2076</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">1x</span>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">1x</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">const LibrdKafkaError = require('../error');
...
&nbsp;
   ...
   * @private
   */
<span class="fstat-no" title="function not covered" >  _g</span>etInternalClient() {
<span class="cstat-no" title="statement not covered" >    return this.#internalClient;</span>
  }
&nbsp;
  /**
   ...
   */
<span class="fstat-no" title="function not covered" >  de</span>pendentAdmin() {
<span class="cstat-no" title="statement not covered" >    return new Admin(null, this);</span>
  }
&nbsp;
  #config() {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!this.#internalConfig)
...
&nbsp;
  ...
  async #clearCacheAndResetPositions() {
    ...
    for (const topicPartition of clearPartitions) {
      ...
      <span class="missing-if-branch" title="else path not taken" >E</span>if (!this.#lastConsumedOffsets.has(key))
        continue;
&nbsp;
      const lastConsumedOffsets = <span class="cstat-no" title="statement not covered" >this.#lastConsumedOffsets.get(key);</span>
      const topicPartitionOffsets = <span class="cstat-no" title="statement not covered" >[</span>
        {
...
<span class="cstat-no" title="statement not covered" >      seeks.push(this.#seekInternal(topicPartitionOffsets));</span>
    }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >      this.#logger.error(`Seek error. This is effectively a fatal error: ${err.stack}`);</span>
    }
  }
&nbsp;
  #unassign(assignment) {
...
&nbsp;
    const assignmentFn = (userAssignment) =&gt; {
      <span class="missing-if-branch" title="if path not taken" >I</span>if (assignmentFnCalled)
...
<span class="cstat-no" title="statement not covered" >        this.#internalClient.incrementalAssign(userAssignment);</span>
<span class="cstat-no" title="statement not covered" >        this.#partitionCount += userAssignment.length;</span>
      }
    };
&nbsp;
...
<span class="cstat-no" title="statement not covered" >          this.#logger.error(`Error from user's rebalance callback: ${e.stack}, `+</span>
                             'continuing with the default rebalance behavior.');
        }
&nbsp;
        <span class="missing-if-branch" title="if path not taken" >I</span>if (alternateAssignment) {
<span class="cstat-no" title="statement not covered" >          assignment = alternateAssignment;</span>
<span class="cstat-no" title="statement not covered" >          assignmentModified = true;</span>
        }
      } else <span class="cstat-no" title="statement not covered" ><span class="missing-if-branch" title="else path not taken" >E</span>if (err.code !== LibrdKafkaError.codes.ERR__ASSIGN_PARTITIONS &amp;&amp; err.code !== LibrdKafkaError.codes.ERR__REVOKE_PARTITIONS) {</span>
<span class="cstat-no" title="statement not covered" >        throw new Error(`Unexpected rebalance_cb error code ${err.code}`);</span>
      }
&nbsp;
    } finally {
      /* Emit the event */
...
&nbsp;
          ...
          <span class="missing-if-branch" title="if path not taken" >I</span>if (checkPendingSeeks &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >!assignmentModified </span>&amp;&amp; <span class="branch-2 cbranch-no" title="branch not covered" >!assignmentFnCalled)</span>
<span class="cstat-no" title="statement not covered" >            assignment = this.#assignAsPerSeekedOffsets(assignment);</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        if (this.#internalClient.isConnected()) {</span>
<span class="cstat-no" title="statement not covered" >          this.#internalClient.emit('rebalance.error', e);</span>
        }
      }
&nbsp;
      /**
      ...
      <span class="missing-if-branch" title="if path not taken" >I</span>if (workersToSpawn !== this.#workers.length) {
<span class="cstat-no" title="statement not covered" >        this.#resolveWorkerTerminationScheduled();</span>
...
&nbsp;
  #kafkaJSToConsumerConfig(kjsConfig) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!kjsConfig || Object.keys(kjsConfig).length === 0) {
<span class="cstat-no" title="statement not covered" >      return {};</span>
    }
&nbsp;
    const disallowedKey = checkAllowedKeys('consumer', kjsConfig);
    <span class="missing-if-branch" title="if path not taken" >I</span>if (disallowedKey !== null) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError(CompatibilityErrorMessages.unsupportedKey(disallowedKey),</span>
...
&nbsp;
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (Object.hasOwn(kjsConfig, 'groupId')) {
      rdKafkaConfig['group.id'] = kjsConfig.groupId;
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(kjsConfig, 'partitionAssigners')) {
<span class="cstat-no" title="statement not covered" >      kjsConfig.partitionAssignors = kjsConfig.partitionAssigners;</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(kjsConfig, 'partitionAssignors')) {
<span class="cstat-no" title="statement not covered" >      if (!Array.isArray(kjsConfig.partitionAssignors)) {</span>
<span class="cstat-no" title="statement not covered" >        throw new error.KafkaJSError(CompatibilityErrorMessages.partitionAssignors(), { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
      }
&nbsp;
<span class="cstat-no" title="statement not covered" >      kjsConfig.partitionAssignors.forEach(<span class="fstat-no" title="function not covered" >as</span>signor =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        if (typeof assignor !== 'string')</span>
<span class="cstat-no" title="statement not covered" >          throw new error.KafkaJSError(CompatibilityErrorMessages.partitionAssignors(), { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
      });
&nbsp;
<span class="cstat-no" title="statement not covered" >      rdKafkaConfig['partition.assignment.strategy'] = kjsConfig.partitionAssignors.join(',');</span>
    } else {
...
<span class="cstat-no" title="statement not covered" >      rdKafkaConfig['session.timeout.ms'] = 30000;</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(kjsConfig, 'rebalanceTimeout')) {
      /* In librdkafka, we use the max poll interval as the rebalance timeout as well. */
<span class="cstat-no" title="statement not covered" >      rdKafkaConfig['max.poll.interval.ms'] = +kjsConfig.rebalanceTimeout;</span>
    } else <span class="missing-if-branch" title="else path not taken" >E</span>if (!rdKafkaConfig['max.poll.interval.ms']) {
      rdKafkaConfig['max.poll.interval.ms'] = 300000; /* librdkafka default */
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(kjsConfig, 'heartbeatInterval')) {
<span class="cstat-no" title="statement not covered" >      rdKafkaConfig['heartbeat.interval.ms'] = kjsConfig.heartbeatInterval;</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(kjsConfig, 'metadataMaxAge')) {
<span class="cstat-no" title="statement not covered" >      rdKafkaConfig['topic.metadata.refresh.interval.ms'] = kjsConfig.metadataMaxAge;</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(kjsConfig, 'allowAutoTopicCreation')) {
<span class="cstat-no" title="statement not covered" >      rdKafkaConfig['allow.auto.create.topics'] = kjsConfig.allowAutoTopicCreation;</span>
    } else {
      rdKafkaConfig['allow.auto.create.topics'] = true;
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(kjsConfig, 'maxBytesPerPartition')) {
<span class="cstat-no" title="statement not covered" >      rdKafkaConfig['max.partition.fetch.bytes'] = kjsConfig.maxBytesPerPartition;</span>
    } else {
...
<span class="cstat-no" title="statement not covered" >      rdKafkaConfig['fetch.wait.max.ms'] = 5000;</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(kjsConfig, 'minBytes')) {
<span class="cstat-no" title="statement not covered" >      rdKafkaConfig['fetch.min.bytes'] = kjsConfig.minBytes;</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(kjsConfig, 'maxBytes')) {
<span class="cstat-no" title="statement not covered" >      rdKafkaConfig['fetch.message.max.bytes'] = kjsConfig.maxBytes;</span>
    } else {
      rdKafkaConfig['fetch.message.max.bytes'] = 10485760;
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(kjsConfig, 'readUncommitted')) {
<span class="cstat-no" title="statement not covered" >      rdKafkaConfig['isolation.level'] = kjsConfig.readUncommitted ? 'read_uncommitted' : 'read_committed';</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(kjsConfig, 'maxInFlightRequests')) {
<span class="cstat-no" title="statement not covered" >      rdKafkaConfig['max.in.flight'] = kjsConfig.maxInFlightRequests;</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(kjsConfig, 'rackId')) {
<span class="cstat-no" title="statement not covered" >      rdKafkaConfig['client.rack'] = kjsConfig.rackId;</span>
    }
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (Object.hasOwn(kjsConfig, 'fromBeginning')) {
      rdKafkaConfig['auto.offset.reset'] = kjsConfig.fromBeginning ? 'earliest' : <span class="branch-1 cbranch-no" title="branch not covered" >'latest';</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(kjsConfig, 'autoCommit')) {
<span class="cstat-no" title="statement not covered" >      rdKafkaConfig['enable.auto.commit'] = kjsConfig.autoCommit;</span>
    } else {
      rdKafkaConfig['enable.auto.commit'] = true;
    }
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (Object.hasOwn(kjsConfig, 'autoCommitInterval')) {
      rdKafkaConfig['auto.commit.interval.ms'] = kjsConfig.autoCommitInterval;
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(kjsConfig, 'autoCommitThreshold')) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError(CompatibilityErrorMessages.runOptionsAutoCommitThreshold(), { code: error.ErrorCodes.ERR__NOT_IMPLEMENTED });</span>
    }
&nbsp;
    /* Set the logger */
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(kjsConfig, 'logger')) {
<span class="cstat-no" title="statement not covered" >      this.#logger = kjsConfig.logger;</span>
    }
&nbsp;
...
&nbsp;
     ...
     */
    <span class="missing-if-branch" title="else path not taken" >E</span>if (Object.hasOwn(this.#userConfig, 'log_level')) {
      this.#logger.setLogLevel(severityToLogLevel[this.#userConfig.log_level]);
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(this.#userConfig, 'debug')) {
<span class="cstat-no" title="statement not covered" >      this.#logger.setLogLevel(logLevel.DEBUG);</span>
    }
&nbsp;
...
&nbsp;
    ...
    rdKafkaConfig['rebalance_cb'] = (err, assignment) =&gt; {
      ...
      this.#rebalanceCallback(err, assignment).catch(<span class="fstat-no" title="function not covered" >e </span>=&gt;
      {
<span class="cstat-no" title="statement not covered" >        if (this.#logger)</span>
<span class="cstat-no" title="statement not covered" >          this.#logger.error(`Error from rebalance callback: ${e.stack}`);</span>
      });
    };
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(this.#userConfig, 'enable.auto.offset.store')) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError(</span>
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!Object.hasOwn(rdKafkaConfig, 'enable.auto.commit')) {
<span class="cstat-no" title="statement not covered" >      this.#autoCommit = true; </span>/* librdkafka default. */
    } else {
...
&nbsp;
     ...
     */
    this.#maxPollIntervalMs = rdKafkaConfig['max.poll.interval.ms'] ?? <span class="branch-1 cbranch-no" title="branch not covered" >300000;</span>
...
&nbsp;
  #readyCb() {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#state !== ConsumerState.CONNECTING) {
...
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError(`Ready callback called in invalid state ${this.#state}`, { code: error.ErrorCodes.ERR__STATE });</span>
    }
...
<span class="cstat-no" title="statement not covered" >    if (this.#state &lt; ConsumerState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      if (!this.#connectionError)</span>
<span class="cstat-no" title="statement not covered" >        this.#connectionError = err;</span>
    } else {
<span class="cstat-no" title="statement not covered" >      this.#logger.error(err, this.#createConsumerBindingMessageMetadata());</span>
    }
  }
&nbsp;
  /**
  ...
  #createHeaders(messageHeaders) {
    let headers;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (messageHeaders) {
<span class="cstat-no" title="statement not covered" >      headers = {};</span>
<span class="cstat-no" title="statement not covered" >      for (const header of messageHeaders) {</span>
<span class="cstat-no" title="statement not covered" >        for (const [key, value] of Object.entries(header)) {</span>
<span class="cstat-no" title="statement not covered" >          if (!Object.hasOwn(headers, key)) {</span>
<span class="cstat-no" title="statement not covered" >            headers[key] = value;</span>
          } else <span class="cstat-no" title="statement not covered" >if (headers[key].constructor === Array) {</span>
<span class="cstat-no" title="statement not covered" >            headers[key].push(value);</span>
          } else {
<span class="cstat-no" title="statement not covered" >            headers[key] = [headers[key], value];</span>
          }
        }
      }
    }
...
&nbsp;
  ...
  #createPayload(message) {
    let key = <span class="cstat-no" title="statement not covered" >message.key;</span>
<span class="cstat-no" title="statement not covered" >    if (typeof key === 'string') {</span>
<span class="cstat-no" title="statement not covered" >      key = Buffer.from(key);</span>
    }
&nbsp;
    let timestamp = <span class="cstat-no" title="statement not covered" >message.timestamp ? String(message.timestamp) : '';</span>
    const headers = <span class="cstat-no" title="statement not covered" >this.#createHeaders(message.headers);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    return {</span>
      topic: message.topic,
      ...
      heartbeat: <span class="fstat-no" title="function not covered" >as</span>ync () =&gt; { /* no op */ },
...
&nbsp;
   ...
   * @private
   */
  #eachBatchPayload_resolveOffsets(payload, offsetToResolve, leaderEpoch = <span class="branch-0 cbranch-no" title="branch not covered" >-1)</span> {
    const offset = +offsetToResolve;
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (isNaN(offset)) {
      /* Not much we can do but throw and log an error. */
      const e = <span class="cstat-no" title="statement not covered" >new error.KafkaJSError(`Invalid offset to resolve: ${offsetToResolve}`, { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (offset &lt;= payload._lastResolvedOffset.offset)
<span class="cstat-no" title="statement not covered" >      return;</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >      this.#logger.error(`Consumer encountered error while storing offset. Error details: ${e}:${e.stack}`, this.#createConsumerBindingMessageMetadata());</span>
    }
  }
&nbsp;
  /**
...
<span class="cstat-no" title="statement not covered" >    if (this.#autoCommit) {</span>
...
<span class="cstat-no" title="statement not covered" >    await this.commitOffsets();</span>
  }
&nbsp;
  /**
   ...
   */
   #increaseMaxSize() {
     <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#messageCacheMaxSize === 1024)
<span class="cstat-no" title="statement not covered" >         return;</span>
     this.#increaseCount++;
...
&nbsp;
      let timestamp = message.timestamp ? String(message.timestamp) : <span class="branch-1 cbranch-no" title="branch not covered" >'';</span>
...
&nbsp;
    const batch = {
      ...
      isEmpty: <span class="fstat-no" title="function not covered" >()</span> =&gt; <span class="cstat-no" title="statement not covered" >false,</span>
      firstOffset: <span class="fstat-no" title="function not covered" >()</span> =&gt; <span class="cstat-no" title="statement not covered" >(messagesConverted[0].offset).toString(),</span>
      lastOffset: <span class="fstat-no" title="function not covered" >()</span> =&gt; <span class="cstat-no" title="statement not covered" >(messagesConverted[messagesConverted.length - 1].offset).toString(),</span>
      offsetLag: <span class="fstat-no" title="function not covered" >()</span> =&gt; <span class="cstat-no" title="statement not covered" >notImplemented(),</span>
      offsetLagLow: <span class="fstat-no" title="function not covered" >()</span> =&gt; <span class="cstat-no" title="statement not covered" >notImplemented(),</span>
    };
&nbsp;
    const returnPayload = {
      batch,
      ...
      isRunning: <span class="fstat-no" title="function not covered" >()</span> =&gt; <span class="cstat-no" title="statement not covered" >this.#running,</span>
      isStale: () =&gt; returnPayload._stale,
      /* NOTE: Probably never to be implemented. Not sure exactly how we'd compute this
       * inexpensively. */
      uncommittedOffsets: <span class="fstat-no" title="function not covered" >()</span> =&gt; <span class="cstat-no" title="statement not covered" >notImplemented(),</span>
    };
&nbsp;
...
&nbsp;
  async #fetchAndResolveWith(takeFromCache, size) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#fetchInProgress) {
<span class="cstat-no" title="statement not covered" >      await this.#fetchInProgress;</span>
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#workerTerminationScheduled.resolved) {
...
&nbsp;
  ...
  async #consumeSingleCached(ppc) {
    const msg = <span class="cstat-no" title="statement not covered" >this.#messageCache.next(ppc);</span>
...
&nbsp;
     ...
     */
<span class="cstat-no" title="statement not covered" >    if (!msg &amp;&amp; this.#messageCache.assignedSize !== 0) {</span>
<span class="cstat-no" title="statement not covered" >      await this.#messageCache.availablePartitions();</span>
      /* Restart with the checks as we might have
       * the cache full. */
<span class="cstat-no" title="statement not covered" >      return null;</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    return this.#fetchAndResolveWith(<span class="fstat-no" title="function not covered" >()</span> =&gt; <span class="cstat-no" title="statement not covered" >this.#messageCache.next(),</span></span>
...
&nbsp;
     ...
     * we should wait for a new partition to be available.
     */
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!msgs &amp;&amp; this.#messageCache.assignedSize !== 0) {
<span class="cstat-no" title="statement not covered" >      await this.#messageCache.availablePartitions();</span>
      /* Restart with the checks as we might have
       * the cache full. */
<span class="cstat-no" title="statement not covered" >      return null;</span>
...
<span class="cstat-no" title="statement not covered" >      this.#internalClient.consume(n, <span class="fstat-no" title="function not covered" >(e</span>rr, messages) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        if (err) {</span>
<span class="cstat-no" title="statement not covered" >          reject(createKafkaJsErrorFromLibRdKafkaError(err));</span>
<span class="cstat-no" title="statement not covered" >          return;</span>
        }
<span class="cstat-no" title="statement not covered" >        resolve(messages);</span>
      });
    });
  }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        for (const partition of topic.partitions) {</span>
<span class="cstat-no" title="statement not covered" >          ret.push({ topic: topic.topic, partition });</span>
        }
      }
    }
    return ret;
...
&nbsp;
   ...
   */
  async connect() {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#state !== ConsumerState.INIT) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError('Connect has already been called elsewhere.', { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >          this.#state = ConsumerState.DISCONNECTED;</span>
          const rejectionError = <span class="cstat-no" title="statement not covered" >this.#connectionError ? this.#connectionError : err;</span>
<span class="cstat-no" title="statement not covered" >          reject(createKafkaJsErrorFromLibRdKafkaError(rejectionError));</span>
        }
      });
    });
  }
&nbsp;
  ...
  async subscribe(subscription) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#state !== ConsumerState.CONNECTED) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError('Subscribe can only be called while connected.', { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (typeof subscription.fromBeginning === 'boolean') {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError(</span>
        CompatibilityErrorMessages.subscribeOptionsFromBeginning(),
        { code: error.ErrorCodes.ERR__INVALID_ARG });
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!Object.hasOwn(subscription, 'topics') &amp;&amp; !Object.hasOwn(subscription, 'topic')) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError(CompatibilityErrorMessages.subscribeOptionsMandatoryMissing(), { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
    }
&nbsp;
    let topics = [];
    if (subscription.topic) {
      topics.push(subscription.topic);
    } else <span class="cstat-no" title="statement not covered" ><span class="missing-if-branch" title="else path not taken" >E</span>if (Array.isArray(subscription.topics)) {</span>
<span class="cstat-no" title="statement not covered" >      topics = subscription.topics;</span>
    } else {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError(CompatibilityErrorMessages.subscribeOptionsMandatoryMissing(), { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
    }
&nbsp;
    topics = topics.map(topic =&gt; {
      if (typeof topic === 'string') {
        return topic;
      } else <span class="cstat-no" title="statement not covered" ><span class="missing-if-branch" title="else path not taken" >E</span>if (topic instanceof RegExp) {</span>
        // Flags are not supported, and librdkafka only considers a regex match if the first character of the regex is ^.
<span class="cstat-no" title="statement not covered" >        if (topic.flags) {</span>
<span class="cstat-no" title="statement not covered" >          throw new error.KafkaJSError(CompatibilityErrorMessages.subscribeOptionsRegexFlag(), { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
        }
        const regexSource = <span class="cstat-no" title="statement not covered" >topic.source;</span>
<span class="cstat-no" title="statement not covered" >        if (regexSource.charAt(0) !== '^')</span>
<span class="cstat-no" title="statement not covered" >          throw new error.KafkaJSError(CompatibilityErrorMessages.subscribeOptionsRegexStart(), { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >        return regexSource;</span>
      } else {
<span class="cstat-no" title="statement not covered" >        throw new error.KafkaJSError('Invalid topic ' + topic + ' (' + typeof topic + '), the topic name has to be a String or a RegExp', { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
      }
    });
&nbsp;
    this.#storedSubscriptions = subscription.replace ? <span class="branch-0 cbranch-no" title="branch not covered" >topics </span>: this.#storedSubscriptions.concat(topics);
    this.#logger.debug(`${subscription.replace ? <span class="branch-0 cbranch-no" title="branch not covered" >'Replacing' </span>: 'Adding'} topics [${topics.join(', ')}] to subscription`, this.#createConsumerBindingMessageMetadata());
    this.#internalClient.subscribe(this.#storedSubscriptions);
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  as</span>ync stop() {
<span class="cstat-no" title="statement not covered" >    notImplemented();</span>
  }
&nbsp;
  /**
  ...
  async run(config) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#state !== ConsumerState.CONNECTED) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError('Run must be called after a successful connect().', { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(config, 'autoCommit')) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError(CompatibilityErrorMessages.runOptionsAutoCommit(), { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(config, 'autoCommitInterval')) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError(CompatibilityErrorMessages.runOptionsAutoCommitInterval(), { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(config, 'autoCommitThreshold')) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError(CompatibilityErrorMessages.runOptionsAutoCommitThreshold(), { code: error.ErrorCodes.ERR__NOT_IMPLEMENTED });</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#running) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError('Consumer is already running.', { code: error.ErrorCodes.ERR__STATE });</span>
    }
    this.#running = true;
&nbsp;
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!Object.hasOwn(config, 'eachBatchAutoResolve')) {
<span class="cstat-no" title="statement not covered" >      configCopy.eachBatchAutoResolve = true;</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!Object.hasOwn(config, 'partitionsConsumedConcurrently')) {
<span class="cstat-no" title="statement not covered" >      configCopy.partitionsConsumedConcurrently = 1;</span>
    }
&nbsp;
...
&nbsp;
  ...
  async #messageProcessor(m, config) {
    let ppc;
<span class="cstat-no" title="statement not covered" >    [m, ppc] = m;</span>
    let key = <span class="cstat-no" title="statement not covered" >partitionKey(m);</span>
    let eachMessageProcessed = <span class="cstat-no" title="statement not covered" >false;</span>
    const payload = <span class="cstat-no" title="statement not covered" >this.#createPayload(m);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    try {</span>
<span class="cstat-no" title="statement not covered" >      this.#lastConsumedOffsets.set(key, m);</span>
<span class="cstat-no" title="statement not covered" >      await config.eachMessage(payload);</span>
<span class="cstat-no" title="statement not covered" >      eachMessageProcessed = true;</span>
    } catch (e) {
...
<span class="cstat-no" title="statement not covered" >    if (!eachMessageProcessed) {</span>
<span class="cstat-no" title="statement not covered" >      this.seek({</span>
        topic: m.topic,
...
<span class="cstat-no" title="statement not covered" >    if (eachMessageProcessed) {</span>
<span class="cstat-no" title="statement not covered" >      try {</span>
<span class="cstat-no" title="statement not covered" >        this.#internalClient._offsetsStoreSingle(m.topic, m.partition, Number(m.offset) + 1, m.leaderEpoch);</span>
      } catch (e) {
        /* Not much we can do, except log the error. */
<span class="cstat-no" title="statement not covered" >        this.#logger.error(`Consumer encountered error while storing offset. Error details: ${JSON.stringify(e)}`, this.#createConsumerBindingMessageMetadata());</span>
      }
    }
&nbsp;
&nbsp;
<span class="cstat-no" title="statement not covered" >    return ppc;</span>
  }
&nbsp;
  /**
...
<span class="cstat-no" title="statement not covered" >      lastOffsetProcessed = payload._lastResolvedOffset;</span>
    }
&nbsp;
...
&nbsp;
    ...
    if (!payload._seeked &amp;&amp; lastOffsetProcessed.offset !== lastOffset) {
      const offsetToSeekTo = lastOffsetProcessed.offset === -1 ? firstMessage.offset : (<span class="branch-1 cbranch-no" title="branch not covered" >lastOffsetProcessed.offset + 1)</span>;
      const leaderEpoch = lastOffsetProcessed.offset === -1 ? firstMessage.leaderEpoch : <span class="branch-1 cbranch-no" title="branch not covered" >lastOffsetProcessed.leaderEpoch;</span>
      this.seek({
...
<span class="cstat-no" title="statement not covered" >    if (ms) {</span>
      let m = <span class="cstat-no" title="statement not covered" >ms[0];</span>
<span class="cstat-no" title="statement not covered" >      if (m.constructor === Array) {</span>
<span class="cstat-no" title="statement not covered" >        m = m[0];</span>
      }
<span class="cstat-no" title="statement not covered" >      ppc = ms[1];</span>
<span class="cstat-no" title="statement not covered" >      if (m &amp;&amp; !this.#lastConsumedOffsets.has(ppc.key)) {</span>
<span class="cstat-no" title="statement not covered" >        this.#lastConsumedOffsets.set(ppc.key, {</span>
          topic: m.topic,
          partition: m.partition,
          offset: m.offset - 1,
        });
...
&nbsp;
  #notifyNonEmpty() {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.#messageCache)
...
&nbsp;
    ...
    nonEmptyAction().catch(<span class="fstat-no" title="function not covered" >(e</span>) =&gt; {
<span class="cstat-no" title="statement not covered" >      this.#logger.error(`Error in queueNonEmptyCb: ${e}`,</span>
        this.#createConsumerBindingMessageMetadata());
    });
  }
  /**
    ...
    while (!this.#workerTerminationScheduled.resolved) {
      try {
        ...
        if (!ms)
          continue;
&nbsp;
        <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#pendingOperations.length) {
<span class="cstat-no" title="statement not covered" >          ppc  = this.#discardMessages(ms, ppc);</span>
...
&nbsp;
        ...
        <span class="missing-if-branch" title="else path not taken" >E</span>if (this.#logger)
...
<span class="cstat-no" title="statement not covered" >      interval = 1;</span>
    ...
    now = hrtime.bigint();
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (now &gt; (maxPollExpiration - 1000000n)) {
...
&nbsp;
      const awakeTime = maxFetchTimeout &lt; cacheExpirationTimeout ?
        maxFetchTimeout : <span class="branch-1 cbranch-no" title="branch not covered" >cacheExpirationTimeout;</span>
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#maxPollIntervalRestart.resolved)
<span class="cstat-no" title="statement not covered" >      this.#maxPollIntervalRestart = new DeferredPromise();</span>
  }
&nbsp;
  /**
...
&nbsp;
  #resolveWorkerTerminationScheduled() {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.#workerTerminationScheduled) {
...
&nbsp;
  ...
  async #runInternal(config) {
    ...
    const perMessageProcessor = config.eachMessage ? <span class="branch-0 cbranch-no" title="branch not covered" >this.#messageProcessor </span>: this.#batchProcessor;
    /* TODO: make this dynamic, based on max batch size / size of last message seen. */
    const maxBatchSize = 32;
    const fetcher = config.eachMessage
      ? <span class="fstat-no" title="function not covered" ><span class="branch-0 cbranch-no" title="branch not covered" >(s</span>avedIdx) =&gt; <span class="cstat-no" title="statement not covered" >this.#consumeSingleCached(savedIdx)</span></span>
...
&nbsp;
        ...
        this.#lastFetchClockNs = hrtime.bigint();
        <span class="missing-if-branch" title="else path not taken" >E</span>if (this.#pendingOperations.length === 0) {
          const workersToSpawn = Math.max(1, Math.min(this.#concurrency, this.#partitionCount));
...
<span class="cstat-no" title="statement not covered" >                    if (this.#logger)</span>
<span class="cstat-no" title="statement not covered" >                      this.#logger.error(`Worker ${i} encountered an error: ${e}:${e.stack}`);</span>
                  }));
&nbsp;
...
&nbsp;
   ...
   * @private
   */
<span class="fstat-no" title="function not covered" >  as</span>ync consume({ timeout } = <span class="branch-0 cbranch-no" title="branch not covered" >{ timeout: 1000 })</span> {
<span class="cstat-no" title="statement not covered" >    if (this.#state !== ConsumerState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError('consume can only be called while connected.', { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (this.#running) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError('consume() and run() cannot be used together.', { code: error.ErrorCodes.ERR__CONFLICT });</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    this.#internalClient.setDefaultConsumeTimeout(timeout);</span>
    let m = <span class="cstat-no" title="statement not covered" >null;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    try {</span>
      const ms = <span class="cstat-no" title="statement not covered" >await this.#consumeN(1);</span>
<span class="cstat-no" title="statement not covered" >      m = ms[0];</span>
    } finally {
<span class="cstat-no" title="statement not covered" >      this.#internalClient.setDefaultConsumeTimeout(undefined);</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    throw new error.KafkaJSError('consume() is not implemented.' + m, { code: error.ErrorCodes.ERR__NOT_IMPLEMENTED });</span>
  }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        err = e;</span>
    ...
    } while (err.code &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >err.code === error.ErrorCodes.ERR__STATE)</span>;
  }
&nbsp;
  /**
   ...
   */
  async commitOffsets(topicPartitions = <span class="branch-0 cbranch-no" title="branch not covered" >null)</span> {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#state !== ConsumerState.CONNECTED) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError('Commit can only be called while connected.', { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
    return new Promise((resolve, reject) =&gt; {
      try {
...
<span class="cstat-no" title="statement not covered" >            reject(createKafkaJsErrorFromLibRdKafkaError(e));</span>
          else
            resolve();
        };
&nbsp;
...
<span class="cstat-no" title="statement not covered" >          <span class="missing-if-branch" title="else path not taken" >E</span>topicPartitions = null;</span>
        this.#internalClient.commitCb(topicPartitions, cb);
      } catch (e) {
<span class="cstat-no" title="statement not covered" >        if (!e.code || e.code !== error.ErrorCodes.ERR__NO_OFFSET)</span>
<span class="cstat-no" title="statement not covered" >          reject(createKafkaJsErrorFromLibRdKafkaError(e));</span>
        else
...
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  as</span>ync committed(topicPartitions = <span class="branch-0 cbranch-no" title="branch not covered" >null,</span> timeout = <span class="branch-0 cbranch-no" title="branch not covered" >-1)</span> {
<span class="cstat-no" title="statement not covered" >    if (this.#state !== ConsumerState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError('Committed can only be called while connected.', { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (!topicPartitions) {</span>
<span class="cstat-no" title="statement not covered" >      topicPartitions = this.assignment();</span>
    }
&nbsp;
    const topicPartitionsRdKafka = <span class="cstat-no" title="statement not covered" >topicPartitions.map(</span>
      topicPartitionOffsetToRdKafka);
&nbsp;
<span class="cstat-no" title="statement not covered" >    return new Promise(<span class="fstat-no" title="function not covered" >(r</span>esolve, reject) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      this.#internalClient.committed(topicPartitionsRdKafka, timeout, <span class="fstat-no" title="function not covered" >(e</span>rr, offsets) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        if (err) {</span>
<span class="cstat-no" title="statement not covered" >          reject(createKafkaJsErrorFromLibRdKafkaError(err));</span>
<span class="cstat-no" title="statement not covered" >          return;</span>
        }
<span class="cstat-no" title="statement not covered" >        resolve(offsets.map(topicPartitionOffsetMetadataToKafkaJS));</span>
      });
    });
  }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >    for (let i = <span class="cstat-no" title="statement not covered" >0;</span> i &lt; assignment.length; i++) {</span>
      const topicPartition = <span class="cstat-no" title="statement not covered" >assignment[i];</span>
      const key = <span class="cstat-no" title="statement not covered" >partitionKey(topicPartition);</span>
<span class="cstat-no" title="statement not covered" >      if (!this.#pendingSeeks.has(key))</span>
<span class="cstat-no" title="statement not covered" >        continue;</span>
&nbsp;
      const tpo = <span class="cstat-no" title="statement not covered" >this.#pendingSeeks.get(key);</span>
<span class="cstat-no" title="statement not covered" >      this.#pendingSeeks.delete(key);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      assignment[i].offset = tpo.offset;</span>
<span class="cstat-no" title="statement not covered" >      assignment[i].leaderEpoch = tpo.leaderEpoch;</span>
    }
...
&nbsp;
  async #seekInternal(topicPartitionOffsets) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (topicPartitionOffsets.length === 0) {
<span class="cstat-no" title="statement not covered" >      return;</span>
    }
&nbsp;
...
&nbsp;
    for (const topicPartitionOffset of topicPartitionOffsets) {
      const key = partitionKey(topicPartitionOffset);
      <span class="missing-if-branch" title="if path not taken" >I</span>if (!assignmentSet.has(key))
<span class="cstat-no" title="statement not covered" >        this.#pendingSeeks.set(key, topicPartitionOffset);</span>
    }
&nbsp;
...
&nbsp;
      ...
      this.#internalClient.seek(topicPartitionOffset, 1000,
        (err) =&gt; {
          <span class="missing-if-branch" title="if path not taken" >I</span>if (err)
<span class="cstat-no" title="statement not covered" >            this.#logger.error(`Error while calling seek from within seekInternal: ${err}`, this.#createConsumerBindingMessageMetadata());</span>
...
&nbsp;
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (offsetsToCommit.length !== 0 &amp;&amp; this.#internalConfig['enable.auto.commit']) {
  ...
  #markBatchPayloadsStale(topicPartitions, isSeek) {
    for (const topicPartition of topicPartitions) {
      ...
      if (this.#topicPartitionToBatchPayload.has(key)) {
        ...
        payload._stale = true;
        <span class="missing-if-branch" title="if path not taken" >I</span>if (isSeek)
<span class="cstat-no" title="statement not covered" >          payload._seeked = true;</span>
      }
    }
  }
&nbsp;
  async #pauseInternal(topicPartitions) {
    ...
    // await Timer.withTimeout(1000);
&nbsp;
<span class="cstat-no" title="statement not covered" >    this.#messageCache.markStale(topicPartitions);</span>
<span class="cstat-no" title="statement not covered" >    this.#internalClient.pause(topicPartitions);</span>
&nbsp;
    const seekOffsets = <span class="cstat-no" title="statement not covered" >[];</span>
<span class="cstat-no" title="statement not covered" >    for (let topicPartition of topicPartitions) {</span>
      const key = <span class="cstat-no" title="statement not covered" >partitionKey(topicPartition);</span>
<span class="cstat-no" title="statement not covered" >      if (this.#lastConsumedOffsets.has(key)) {</span>
        const seekOffset = <span class="cstat-no" title="statement not covered" >this.#lastConsumedOffsets.get(key);</span>
        const topicPartitionOffset = <span class="cstat-no" title="statement not covered" >{</span>
          topic: topicPartition.topic,
          partition: topicPartition.partition,
          offset: seekOffset.offset + 1,
          leaderEpoch: seekOffset.leaderEpoch,
        };
<span class="cstat-no" title="statement not covered" >        seekOffsets.push(topicPartitionOffset);</span>
      }
    }
<span class="cstat-no" title="statement not covered" >    if (seekOffsets.length) {</span>
<span class="cstat-no" title="statement not covered" >      await this.#seekInternal(seekOffsets);</span>
    }
  }
&nbsp;
  async #resumeInternal(topicPartitions) {
    ...
    // await Timer.withTimeout(1000);
<span class="cstat-no" title="statement not covered" >    this.#internalClient.resume(topicPartitions);</span>
  }
&nbsp;
  /**
  ...
  seek(topicPartitionOffset) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#state !== ConsumerState.CONNECTED) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError('Seek can only be called while connected.', { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
    const rdKafkaTopicPartitionOffset =
      topicPartitionOffsetToRdKafka(topicPartitionOffset);
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (typeof rdKafkaTopicPartitionOffset.topic !== 'string') {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError('Topic must be a string.', { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (isNaN(rdKafkaTopicPartitionOffset.offset) || (rdKafkaTopicPartitionOffset.offset &lt; 0 &amp;&amp; <span class="branch-2 cbranch-no" title="branch not covered" >rdKafkaTopicPartitionOffset.offset !== -2 </span>&amp;&amp; <span class="branch-3 cbranch-no" title="branch not covered" >rdKafkaTopicPartitionOffset.offset !== -3)</span>) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError('Offset must be &gt;= 0, or a special value.', { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
    }
&nbsp;
...
&nbsp;
<span class="fstat-no" title="function not covered" >  as</span>ync describeGroup() {
<span class="cstat-no" title="statement not covered" >    notImplemented();</span>
  }
&nbsp;
  /**
  ...
  assignment() {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#state !== ConsumerState.CONNECTED) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError('Assignment can only be called while connected.', { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
...
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  re</span>balanceProtocol() {
<span class="cstat-no" title="statement not covered" >    if (this.#state !== ConsumerState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      return "NONE";</span>
    }
<span class="cstat-no" title="statement not covered" >    return this.#internalClient.rebalanceProtocol();</span>
  }
&nbsp;
  /**
...
<span class="cstat-no" title="statement not covered" >    return this.#internalClient.assignments()</span>
      .filter(<span class="fstat-no" title="function not covered" >(p</span>artition) =&gt; <span class="cstat-no" title="statement not covered" >partition.topic === topic)</span>
      .map(<span class="fstat-no" title="function not covered" >(t</span>po) =&gt; <span class="cstat-no" title="statement not covered" >tpo.partition)</span>;
  }
&nbsp;
  /**
   ...
   */
<span class="fstat-no" title="function not covered" >  pa</span>use(topics) {
<span class="cstat-no" title="statement not covered" >    if (this.#state !== ConsumerState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError('Pause can only be called while connected.', { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    this.#logger.debug(`Pausing ${topics.length} topics`, this.#createConsumerBindingMessageMetadata());</span>
&nbsp;
    const toppars = <span class="cstat-no" title="statement not covered" >[];</span>
<span class="cstat-no" title="statement not covered" >    for (let topic of topics) {</span>
<span class="cstat-no" title="statement not covered" >      if (typeof topic.topic !== 'string') {</span>
<span class="cstat-no" title="statement not covered" >        throw new error.KafkaJSError('Topic must be a string.', { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
      }
&nbsp;
      const toppar = <span class="cstat-no" title="statement not covered" >{ topic: topic.topic };</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      if (!topic.partitions) {</span>
<span class="cstat-no" title="statement not covered" >        toppar.partitions = this.#getAllAssignedPartition(topic.topic);</span>
      } else {
        /* TODO: add a check here to make sure we own each partition */
<span class="cstat-no" title="statement not covered" >        toppar.partitions = [...topic.partitions];</span>
      }
&nbsp;
<span class="cstat-no" title="statement not covered" >      toppars.push(toppar);</span>
    }
&nbsp;
    const flattenedToppars = <span class="cstat-no" title="statement not covered" >this.#flattenTopicPartitions(toppars);</span>
<span class="cstat-no" title="statement not covered" >    if (flattenedToppars.length === 0) {</span>
<span class="cstat-no" title="statement not covered" >      return;</span>
    }
&nbsp;
    /* If anyone's using eachBatch, mark the batch as stale. */
<span class="cstat-no" title="statement not covered" >    this.#markBatchPayloadsStale(flattenedToppars);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    flattenedToppars.forEach(</span>
<span class="fstat-no" title="function not covered" >      to</span>picPartition =&gt; <span class="cstat-no" title="statement not covered" >this.#pausedPartitions.set(</span>
        partitionKey(topicPartition),
        topicPartition));
&nbsp;
<span class="cstat-no" title="statement not covered" >    this.#addPendingOperation(<span class="fstat-no" title="function not covered" >()</span> =&gt;</span>
<span class="cstat-no" title="statement not covered" >        this.#pauseInternal(flattenedToppars))</span>;
&nbsp;
    /* Note: we don't use flattenedToppars here because resume flattens them again. */
<span class="cstat-no" title="statement not covered" >    return <span class="fstat-no" title="function not covered" >()</span> =&gt; <span class="cstat-no" title="statement not covered" >this.resume(toppars);</span></span>
  }
&nbsp;
  /**
   ...
   */
<span class="fstat-no" title="function not covered" >  pa</span>used() {
    const topicToPartitions = <span class="cstat-no" title="statement not covered" >Array</span>
      .from(this.#pausedPartitions.values())
      .reduce(
<span class="fstat-no" title="function not covered" >        (a</span>cc, { topic, partition }) =&gt; {
<span class="cstat-no" title="statement not covered" >          if (!acc[topic]) {</span>
<span class="cstat-no" title="statement not covered" >            acc[topic] = [];</span>
          }
<span class="cstat-no" title="statement not covered" >          acc[topic].push(partition);</span>
<span class="cstat-no" title="statement not covered" >          return acc;</span>
        },
        {});
<span class="cstat-no" title="statement not covered" >    return Array.from(Object.entries(topicToPartitions), <span class="fstat-no" title="function not covered" >([</span>topic, partitions]) =&gt; (<span class="cstat-no" title="statement not covered" >{ topic, partitions })</span>);</span>
  }
&nbsp;
&nbsp;
  /**
   ...
   */
<span class="fstat-no" title="function not covered" >  re</span>sume(topics) {
<span class="cstat-no" title="statement not covered" >    if (this.#state !== ConsumerState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError('Resume can only be called while connected.', { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    this.#logger.debug(`Resuming ${topics.length} topics`, this.#createConsumerBindingMessageMetadata());</span>
&nbsp;
    const toppars = <span class="cstat-no" title="statement not covered" >[];</span>
<span class="cstat-no" title="statement not covered" >    for (let topic of topics) {</span>
<span class="cstat-no" title="statement not covered" >      if (typeof topic.topic !== 'string') {</span>
...
<span class="cstat-no" title="statement not covered" >    flattenedToppars.map(partitionKey).</span>
      forEach(<span class="fstat-no" title="function not covered" >ke</span>y =&gt; <span class="cstat-no" title="statement not covered" >this.#pausedPartitions.delete(key))</span>;
&nbsp;
<span class="cstat-no" title="statement not covered" >    this.#addPendingOperation(<span class="fstat-no" title="function not covered" >()</span> =&gt;</span>
<span class="cstat-no" title="statement not covered" >      this.#resumeInternal(flattenedToppars))</span>;
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  on</span>(/* eventName, listener */) {
<span class="cstat-no" title="statement not covered" >    notImplemented();</span>
  }
&nbsp;
  /**
   ...
   */
<span class="fstat-no" title="function not covered" >  lo</span>gger() {
<span class="cstat-no" title="statement not covered" >    return this.#logger;</span>
  }
&nbsp;
<span class="fstat-no" title="function not covered" >  ge</span>t events() {
<span class="cstat-no" title="statement not covered" >    notImplemented();</span>
<span class="cstat-no" title="statement not covered" >    return null;</span>
  }
&nbsp;
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#state &gt;= ConsumerState.DISCONNECTING) {
<span class="cstat-no" title="statement not covered" >      return;</span>
    }
&nbsp;
