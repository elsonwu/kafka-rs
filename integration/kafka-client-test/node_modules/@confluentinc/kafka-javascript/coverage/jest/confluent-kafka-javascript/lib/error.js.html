...
<head>
    <title>Code coverage report for confluent-kafka-javascript/lib/error.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">confluent-kafka-javascript/lib</a> error.js</h1>
        <div class='clearfix'>
...
<a name='L476'></a><a href='#L476'>476</a></td><td class="line-coverage quiet"><span class="cline-any cline-neutral">&nbsp;</span>
...
function LibrdKafkaError(e) {
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!(this instanceof LibrdKafkaError)) {
<span class="cstat-no" title="statement not covered" >    return new LibrdKafkaError(e);</span>
  }
&nbsp;
  if (typeof e === 'number') {
    ...
    this.code = e.code;
    this.errno = e.code;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (e.code &gt;= LibrdKafkaError.codes.ERR__END) {
<span class="cstat-no" title="statement not covered" >      this.origin = 'local';</span>
    } else {
      this.origin = 'kafka';
    }
    Error.captureStackTrace(this, this.constructor);
...
<span class="cstat-no" title="statement not covered" >      origin = 'unknown';</span>
<span class="cstat-no" title="statement not covered" >      msg = message.toLowerCase();</span>
    }
&nbsp;
    // special cases
    <span class="missing-if-branch" title="if path not taken" >I</span>if (msg === 'consumer is disconnected' || msg === 'producer is disconnected') {
<span class="cstat-no" title="statement not covered" >      this.origin = 'local';</span>
<span class="cstat-no" title="statement not covered" >      this.code = LibrdKafkaError.codes.ERR__STATE;</span>
<span class="cstat-no" title="statement not covered" >      this.errno = this.code;</span>
<span class="cstat-no" title="statement not covered" >      this.message = msg;</span>
    } else {
      this.origin = origin;
      this.message = msg;
      this.code = typeof e.code === 'number' ? e.code : <span class="branch-1 cbranch-no" title="branch not covered" >-1;</span>
...
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(e, 'isFatal')) <span class="cstat-no" title="statement not covered" >this.isFatal = e.isFatal;</span>
  <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(e, 'isRetriable')) <span class="cstat-no" title="statement not covered" >this.isRetriable = e.isRetriable;</span>
  <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(e, 'isTxnRequiresAbort')) <span class="cstat-no" title="statement not covered" >this.isTxnRequiresAbort = e.isTxnRequiresAbort;</span>
&nbsp;
}
&nbsp;
...
function errorWrap(errorCode, intIsError) {
  ...
  <span class="missing-if-branch" title="else path not taken" >E</span>if (intIsError) {
