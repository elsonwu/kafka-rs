...
<head>
    <title>Code coverage report for confluent-kafka-javascript/lib/kafkajs/_admin.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../../index.html">All files</a> / <a href="index.html">confluent-kafka-javascript/lib/kafkajs</a> _admin.js</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">22.34% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>80/358</span>
            </div>
        
            
            <div class='fl pad1y space-right2'>
                <span class="strong">14.77% </span>
                <span class="quiet">Branches</span>
                <span class='fraction'>26/176</span>
            </div>
            ...
            <div class='fl pad1y space-right2'>
                ...
                <span class='fraction'>80/339</span>
            </div>
...
<a name='L1029'></a><a href='#L1029'>1029</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">1x</span>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">const { OffsetSpec } = require('../admin');
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (disallowedKey) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError(CompatibilityErrorMessages.unsupportedKey(disallowedKey), { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
    }
&nbsp;
...
&nbsp;
  #readyCb() {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#state !== AdminState.CONNECTING) {
...
<span class="cstat-no" title="statement not covered" >    if (this.#state &lt; AdminState.CONNECTED) {</span>
...
<span class="cstat-no" title="statement not covered" >      this.#logger.error(`Error: ${err.message}`, this.#createAdminBindingMessageMetadata());</span>
    }
  }
&nbsp;
  /**
   ...
   */
  async connect() {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#state !== AdminState.INIT) {
      ...
      try {
        ...
        if (!this.#existingClient) {
          ...
          this.#internalClient = RdKafka.AdminClient.create(config, {
            ...
            'event.log': <span class="fstat-no" title="function not covered" >(m</span>sg) =&gt; <span class="cstat-no" title="statement not covered" >loggerTrampoline(msg, this.#logger),</span>
          });
        } else <span class="missing-if-branch" title="else path not taken" >E</span>{
          const underlyingClient = <span class="cstat-no" title="statement not covered" >this.#existingClient._getInternalClient();</span>
<span class="cstat-no" title="statement not covered" >          if (!underlyingClient) {</span>
<span class="cstat-no" title="statement not covered" >            throw new error.KafkaJSError("Underlying client is not connected.", { code: error.ErrorCodes.ERR__STATE });</span>
          }
<span class="cstat-no" title="statement not covered" >          this.#logger = this.#existingClient.logger();</span>
<span class="cstat-no" title="statement not covered" >          this.#internalClient = RdKafka.AdminClient.createFrom(underlyingClient, {</span>
            'ready': this.#readyCb.bind(this),
          });
        }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        this.#state = AdminState.DISCONNECTED;</span>
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#state &gt;= AdminState.DISCONNECTING) {
<span class="cstat-no" title="statement not covered" >      return;</span>
    }
&nbsp;
...
&nbsp;
  ...
  #topicConfigToRdKafka(topic) {
    ...
    topicConfig.num_partitions = topic.numPartitions ?? <span class="branch-1 cbranch-no" title="branch not covered" >-1;</span>
    topicConfig.replication_factor = topic.replicationFactor ?? -1;
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(topic, "replicaAssignment")) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("replicaAssignment is not yet implemented.", { code: error.ErrorCodes.ERR__NOT_IMPLEMENTED });</span>
    }
&nbsp;
    topicConfig.config = {};
    topic.configEntries = topic.configEntries ?? [];
    for (const configEntry of topic.configEntries) {
<span class="cstat-no" title="statement not covered" >      topicConfig.config[configEntry.name] = configEntry.value;</span>
    }
&nbsp;
    return topicConfig;
  }
&nbsp;
  ...
  async createTopics(options) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (this.#state !== AdminState.CONNECTED) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("Admin client is not connected.", { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(options, "validateOnly")) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("validateOnly is not yet implemented.", { code: error.ErrorCodes.ERR__NOT_IMPLEMENTED });</span>
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Object.hasOwn(options, "waitForLeaders")) {
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("waitForLeaders is not yet implemented.", { code: error.ErrorCodes.ERR__NOT_IMPLEMENTED });</span>
    }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >              if (err.code === error.ErrorCodes.ERR_TOPIC_ALREADY_EXISTS) {</span>
<span class="cstat-no" title="statement not covered" >                allTopicsCreated = false;</span>
<span class="cstat-no" title="statement not covered" >                resolve();</span>
<span class="cstat-no" title="statement not covered" >                return;</span>
              }
              const e = <span class="cstat-no" title="statement not covered" >createKafkaJsErrorFromLibRdKafkaError(err);</span>
              const createTopicError = <span class="cstat-no" title="statement not covered" >new error.KafkaJSCreateTopicError(e, topicConfig.topic, e /* includes the properties */);</span>
<span class="cstat-no" title="statement not covered" >              errors.push(createTopicError);</span>
<span class="cstat-no" title="statement not covered" >              resolve(); </span>// Don't reject this promise, instead, look at the errors array later.
            } else {
              resolve();
            }
          });
...
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSAggregateError("Topic creation errors", errors);</span>
    }
...
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  as</span>ync deleteTopics(options) {
<span class="cstat-no" title="statement not covered" >    if (this.#state !== AdminState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("Admin client is not connected.", { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    return Promise.all(</span>
      options.topics.map(<span class="fstat-no" title="function not covered" >to</span>pic =&gt; <span class="cstat-no" title="statement not covered" >new Promise(<span class="fstat-no" title="function not covered" >(r</span>esolve, reject) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        this.#internalClient.deleteTopic(topic, options.timeout ?? 5000, <span class="fstat-no" title="function not covered" >er</span>r =&gt; {</span>
...
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  as</span>ync listGroups(options = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
<span class="cstat-no" title="statement not covered" >    if (this.#state !== AdminState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("Admin client is not connected.", { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >      this.#internalClient.listGroups(options, <span class="fstat-no" title="function not covered" >(e</span>rr, groups) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        if (err) {</span>
<span class="cstat-no" title="statement not covered" >          reject(createKafkaJsErrorFromLibRdKafkaError(err));</span>
        } else {
<span class="cstat-no" title="statement not covered" >          resolve(groups);</span>
        }
      });
    });
  }
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  as</span>ync describeGroups(groups, options = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
<span class="cstat-no" title="statement not covered" >    if (this.#state !== AdminState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("Admin client is not connected.", { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    return new Promise(<span class="fstat-no" title="function not covered" >(r</span>esolve, reject) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      this.#internalClient.describeGroups(groups, options, <span class="fstat-no" title="function not covered" >(e</span>rr, descriptions) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        if (err) {</span>
<span class="cstat-no" title="statement not covered" >          reject(createKafkaJsErrorFromLibRdKafkaError(err));</span>
        } else {
<span class="cstat-no" title="statement not covered" >          resolve(descriptions);</span>
        }
      });
    });
  }
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  as</span>ync deleteGroups(groups, options = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
<span class="cstat-no" title="statement not covered" >    if (this.#state !== AdminState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("Admin client is not connected.", { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    return new Promise(<span class="fstat-no" title="function not covered" >(r</span>esolve, reject) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      this.#internalClient.deleteGroups(groups, options, <span class="fstat-no" title="function not covered" >(e</span>rr, reports) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        if (err) {</span>
<span class="cstat-no" title="statement not covered" >          reject(createKafkaJsErrorFromLibRdKafkaError(err));</span>
<span class="cstat-no" title="statement not covered" >          return;</span>
        }
&nbsp;
        ...
        let errorsPresent = <span class="cstat-no" title="statement not covered" >false;</span>
<span class="cstat-no" title="statement not covered" >        reports = reports.map(<span class="fstat-no" title="function not covered" >gr</span>oupReport =&gt; {</span>
<span class="cstat-no" title="statement not covered" >          if (groupReport.error) {</span>
<span class="cstat-no" title="statement not covered" >            errorsPresent = true;</span>
<span class="cstat-no" title="statement not covered" >            groupReport.error = createKafkaJsErrorFromLibRdKafkaError(groupReport.error);</span>
          }
<span class="cstat-no" title="statement not covered" >          return groupReport;</span>
        });
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (errorsPresent) {</span>
<span class="cstat-no" title="statement not covered" >          reject(new error.KafkaJSDeleteGroupsError('Error in DeleteGroups', reports));</span>
<span class="cstat-no" title="statement not covered" >          return;</span>
        }
<span class="cstat-no" title="statement not covered" >        resolve(reports);</span>
      });
    });
  }
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  as</span>ync listTopics(options = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
<span class="cstat-no" title="statement not covered" >    if (this.#state !== AdminState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("Admin client is not connected.", { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    return new Promise(<span class="fstat-no" title="function not covered" >(r</span>esolve, reject) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      this.#internalClient.listTopics(options, <span class="fstat-no" title="function not covered" >(e</span>rr, topics) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        if (err) {</span>
<span class="cstat-no" title="statement not covered" >          reject(createKafkaJsErrorFromLibRdKafkaError(err));</span>
        } else {
<span class="cstat-no" title="statement not covered" >          resolve(topics);</span>
        }
      });
    });
  }
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  as</span>ync fetchOffsets(options = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
<span class="cstat-no" title="statement not covered" >    if (this.#state !== AdminState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("Admin client is not connected.", { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (Object.hasOwn(options, "resolveOffsets")) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("resolveOffsets is not yet implemented.", { code: error.ErrorCodes.ERR__NOT_IMPLEMENTED });</span>
    }
&nbsp;
    const { groupId, topics } = <span class="cstat-no" title="statement not covered" >options;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (!groupId) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("groupId is required.", { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
    }
&nbsp;
    let partitions = <span class="cstat-no" title="statement not covered" >null;</span>
    let originalTopics = <span class="cstat-no" title="statement not covered" >null;</span>
&nbsp;
    /*
    ...
    */
<span class="cstat-no" title="statement not covered" >    if (topics &amp;&amp; Array.isArray(topics)) {</span>
<span class="cstat-no" title="statement not covered" >      if (typeof topics[0] === 'string') {</span>
<span class="cstat-no" title="statement not covered" >        originalTopics = topics;</span>
<span class="cstat-no" title="statement not covered" >        partitions = null;</span>
      } else <span class="cstat-no" title="statement not covered" >if (typeof topics[0] === 'object' &amp;&amp; Array.isArray(topics[0].partitions)) {</span>
<span class="cstat-no" title="statement not covered" >        partitions = topics.flatMap(<span class="fstat-no" title="function not covered" >to</span>pic =&gt; <span class="cstat-no" title="statement not covered" >topic.partitions.map(<span class="fstat-no" title="function not covered" >pa</span>rtition =&gt; (<span class="cstat-no" title="statement not covered" >{</span></span></span>
          topic: topic.topic,
          partition
        })));
      } else {
<span class="cstat-no" title="statement not covered" >        throw new error.KafkaJSError("Invalid topics format.", { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
      }
    }
&nbsp;
    const listGroupOffsets = <span class="cstat-no" title="statement not covered" >[{</span>
      groupId,
      partitions
    }];
&nbsp;
&nbsp;
<span class="cstat-no" title="statement not covered" >    return new Promise(<span class="fstat-no" title="function not covered" >(r</span>esolve, reject) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      this.#internalClient.listConsumerGroupOffsets(listGroupOffsets, options, <span class="fstat-no" title="function not covered" >(e</span>rr, offsets) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        if (err) {</span>
<span class="cstat-no" title="statement not covered" >          reject(createKafkaJsErrorFromLibRdKafkaError(err));</span>
        } else {
&nbsp;
           ...
           */
          const topicPartitionMap = <span class="cstat-no" title="statement not covered" >new Map();</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >          if (offsets.length !== 1) {</span>
<span class="cstat-no" title="statement not covered" >            reject(new error.KafkaJSError("Unexpected number of group results."));</span>
<span class="cstat-no" title="statement not covered" >            return;</span>
          }
&nbsp;
          const groupResult = <span class="cstat-no" title="statement not covered" >offsets[0];</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >          if (groupResult.error) {</span>
<span class="cstat-no" title="statement not covered" >            reject(createKafkaJsErrorFromLibRdKafkaError(groupResult.error));</span>
<span class="cstat-no" title="statement not covered" >            return;</span>
          }
&nbsp;
          // Traverse the partitions and group them by topic
<span class="cstat-no" title="statement not covered" >          groupResult.partitions.forEach(<span class="fstat-no" title="function not covered" >pa</span>rtitionObj =&gt; {</span>
            const { topic, partition, offset, leaderEpoch, metadata, error } = <span class="cstat-no" title="statement not covered" >partitionObj;</span>
            const fetchOffsetsPartition = <span class="cstat-no" title="statement not covered" >{</span>
              partition: partition,
...
<span class="cstat-no" title="statement not covered" >            if (!topicPartitionMap.has(topic)) {</span>
<span class="cstat-no" title="statement not covered" >              topicPartitionMap.set(topic, []);</span>
            }
<span class="cstat-no" title="statement not covered" >            topicPartitionMap.get(topic).push(fetchOffsetsPartition);</span>
          });
&nbsp;
          // Convert the map back to the desired array format
          let convertedOffsets = <span class="cstat-no" title="statement not covered" >Array.from(topicPartitionMap, <span class="fstat-no" title="function not covered" >([</span>topic, partitions]) =&gt; (<span class="cstat-no" title="statement not covered" >{</span></span>
            topic,
            partitions
          }));
&nbsp;
<span class="cstat-no" title="statement not covered" >          if (originalTopics !== null) {</span>
<span class="cstat-no" title="statement not covered" >            convertedOffsets = convertedOffsets.filter(<span class="fstat-no" title="function not covered" >co</span>nvertedOffset =&gt; <span class="cstat-no" title="statement not covered" >originalTopics.includes(convertedOffset.topic))</span>;</span>
          }
<span class="cstat-no" title="statement not covered" >          resolve(convertedOffsets);</span>
        }
      });
    });
  }
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  as</span>ync deleteTopicRecords(options = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
<span class="cstat-no" title="statement not covered" >    if (this.#state !== AdminState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("Admin client is not connected.", { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (!Object.hasOwn(options, 'topic') || !Object.hasOwn(options, 'partitions') || !Array.isArray(options.partitions)) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("Options must include 'topic' and 'partitions', and 'partitions' must be an array.", { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
    }
&nbsp;
    const { topic, partitions } = <span class="cstat-no" title="statement not covered" >options;</span>
&nbsp;
...
&nbsp;
<span class="cstat-no" title="statement not covered" >    for (const partition of partitions) {</span>
<span class="cstat-no" title="statement not covered" >      if (partition.offset === null || partition.offset === undefined) {</span>
<span class="cstat-no" title="statement not covered" >        throw new error.KafkaJSError("Each partition must have a valid offset.", { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
      }
&nbsp;
      const offset = <span class="cstat-no" title="statement not covered" >+partition.offset;</span>
<span class="cstat-no" title="statement not covered" >      if (isNaN(offset)) {</span>
<span class="cstat-no" title="statement not covered" >        throw new error.KafkaJSError("Offset must be a valid number.", { code: error.ErrorCodes.ERR__INVALID_ARG });</span>
      }
&nbsp;
<span class="cstat-no" title="statement not covered" >      topicPartitionOffsets.push({</span>
        topic,
...
&nbsp;
<span class="cstat-no" title="statement not covered" >    return new Promise(<span class="fstat-no" title="function not covered" >(r</span>esolve, reject) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      this.#internalClient.deleteRecords(topicPartitionOffsets, options, <span class="fstat-no" title="function not covered" >(e</span>rr, results) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        if (err) {</span>
<span class="cstat-no" title="statement not covered" >          reject(createKafkaJsErrorFromLibRdKafkaError(err));</span>
<span class="cstat-no" title="statement not covered" >          return;</span>
        }
&nbsp;
        let errorsPresent = <span class="cstat-no" title="statement not covered" >false;</span>
<span class="cstat-no" title="statement not covered" >        results = results.map(<span class="fstat-no" title="function not covered" >re</span>sult =&gt; {</span>
<span class="cstat-no" title="statement not covered" >          if (result.error) {</span>
<span class="cstat-no" title="statement not covered" >            errorsPresent = true;</span>
<span class="cstat-no" title="statement not covered" >            result.error = createKafkaJsErrorFromLibRdKafkaError(result.error);</span>
          }
...
<span class="cstat-no" title="statement not covered" >          {</span>
            ...
            partitions:
              results.filter(<span class="fstat-no" title="function not covered" >re</span>sult =&gt; <span class="cstat-no" title="statement not covered" >result.error)</span>.map(<span class="fstat-no" title="function not covered" >re</span>sult =&gt; (<span class="cstat-no" title="statement not covered" >{</span>
                partition: result.partition,
                offset: String(result.lowWatermark),
                error: result.error,
              }))
          };
<span class="cstat-no" title="statement not covered" >          reject(new error.KafkaJSDeleteTopicRecordsError(partitionsWithError));</span>
<span class="cstat-no" title="statement not covered" >          return;</span>
        }
...
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  as</span>ync fetchTopicMetadata(options = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
<span class="cstat-no" title="statement not covered" >    if (this.#state !== AdminState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("Admin client is not connected.", { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
    let topics = <span class="cstat-no" title="statement not covered" >options.topics;</span>
<span class="cstat-no" title="statement not covered" >    if (!Object.hasOwn(options, 'topics')) {</span>
<span class="cstat-no" title="statement not covered" >      try {</span>
<span class="cstat-no" title="statement not covered" >        topics = await this.listTopics();</span>
      } catch (err) {
<span class="cstat-no" title="statement not covered" >        throw createKafkaJsErrorFromLibRdKafkaError(err);</span>
      }
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    return new Promise(<span class="fstat-no" title="function not covered" >(r</span>esolve, reject) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      this.#internalClient.describeTopics(topics, options, <span class="fstat-no" title="function not covered" >(e</span>rr, metadata) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        if (err) {</span>
<span class="cstat-no" title="statement not covered" >          reject(createKafkaJsErrorFromLibRdKafkaError(err));</span>
        } else {
&nbsp;
          let errs = <span class="cstat-no" title="statement not covered" >metadata.filter(<span class="fstat-no" title="function not covered" >to</span>pic =&gt; <span class="cstat-no" title="statement not covered" >topic.error)</span>;</span>
<span class="cstat-no" title="statement not covered" >          if (errs.length &gt; 0) {</span>
<span class="cstat-no" title="statement not covered" >            reject(createKafkaJsErrorFromLibRdKafkaError(errs[0].error));</span>
<span class="cstat-no" title="statement not covered" >            return;</span>
          }
          const convertedMetadata = <span class="cstat-no" title="statement not covered" >metadata.map(<span class="fstat-no" title="function not covered" >to</span>pic =&gt; (<span class="cstat-no" title="statement not covered" >{</span></span>
            name: topic.name,
            topicId: topic.topicId,
            isInternal: topic.isInternal,
            partitions: topic.partitions.map(<span class="fstat-no" title="function not covered" >pa</span>rtition =&gt; (<span class="cstat-no" title="statement not covered" >{</span>
              partitionErrorCode: error.ErrorCodes.ERR_NO_ERROR,
              partitionId: partition.partition,
              leader: partition.leader.id,
              leaderNode: partition.leader,
              replicas: partition.replicas.map(<span class="fstat-no" title="function not covered" >re</span>plica =&gt; <span class="cstat-no" title="statement not covered" >replica.id)</span>,
              replicaNodes: partition.replicas,
              isr: partition.isr.map(<span class="fstat-no" title="function not covered" >is</span>rNode =&gt; <span class="cstat-no" title="statement not covered" >isrNode.id)</span>,
              isrNodes: partition.isr
            })),
            authorizedOperations: topic.authorizedOperations
          }));
&nbsp;
<span class="cstat-no" title="statement not covered" >          resolve(convertedMetadata);</span>
        }
      });
    });
  }
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  as</span>ync fetchTopicOffsets(topic, options = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
<span class="cstat-no" title="statement not covered" >    if (this.#state !== AdminState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("Admin client is not connected.", { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (!Object.hasOwn(options, 'timeout')) {</span>
...
<span class="cstat-no" title="statement not covered" >      startTime = hrtime.bigint();</span>
<span class="cstat-no" title="statement not covered" >      topicData = await this.fetchTopicMetadata({ topics: [topic], timeout: options.timeout });</span>
<span class="cstat-no" title="statement not covered" >      endTime = hrtime.bigint();</span>
<span class="cstat-no" title="statement not covered" >      timeTaken = Number(endTime - startTime) / 1e6; </span>// Convert nanoseconds to milliseconds
&nbsp;
      // Adjust timeout for the next request
<span class="cstat-no" title="statement not covered" >      options.timeout -= timeTaken;</span>
<span class="cstat-no" title="statement not covered" >      if (options.timeout &lt;= 0) {</span>
<span class="cstat-no" title="statement not covered" >        throw new error.KafkaJSError("Timeout exceeded while fetching topic metadata.", { code: error.ErrorCodes.ERR__TIMED_OUT });</span>
      }
    } catch (err) {
<span class="cstat-no" title="statement not covered" >      throw new createKafkaJsErrorFromLibRdKafkaError(err);</span>
    }
&nbsp;
    const partitionIds = <span class="cstat-no" title="statement not covered" >topicData.flatMap(<span class="fstat-no" title="function not covered" >to</span>pic =&gt;</span>
<span class="cstat-no" title="statement not covered" >      topic.partitions.map(<span class="fstat-no" title="function not covered" >pa</span>rtition =&gt; <span class="cstat-no" title="statement not covered" >partition.partitionId)</span></span>
    );
&nbsp;
    const topicPartitionOffsetsLatest = <span class="cstat-no" title="statement not covered" >partitionIds.map(<span class="fstat-no" title="function not covered" >pa</span>rtitionId =&gt; (<span class="cstat-no" title="statement not covered" >{</span></span>
      topic,
      partition: partitionId,
      offset: OffsetSpec.LATEST
    }));
&nbsp;
    const topicPartitionOffsetsEarliest = <span class="cstat-no" title="statement not covered" >partitionIds.map(<span class="fstat-no" title="function not covered" >pa</span>rtitionId =&gt; (<span class="cstat-no" title="statement not covered" >{</span></span>
      topic,
      partition: partitionId,
...
<span class="cstat-no" title="statement not covered" >      startTime = hrtime.bigint();</span>
      const latestOffsets = <span class="cstat-no" title="statement not covered" >await this.#listOffsets(topicPartitionOffsetsLatest, options);</span>
<span class="cstat-no" title="statement not covered" >      endTime = hrtime.bigint();</span>
<span class="cstat-no" title="statement not covered" >      timeTaken = Number(endTime - startTime) / 1e6; </span>// Convert nanoseconds to milliseconds
&nbsp;
      // Adjust timeout for the next request
<span class="cstat-no" title="statement not covered" >      options.timeout -= timeTaken;</span>
<span class="cstat-no" title="statement not covered" >      if (options.timeout &lt;= 0) {</span>
<span class="cstat-no" title="statement not covered" >        throw new error.KafkaJSError("Timeout exceeded while fetching latest offsets.", { code: error.ErrorCodes.ERR__TIMED_OUT });</span>
      }
&nbsp;
      // Measure time taken for listOffsets (earliest)
<span class="cstat-no" title="statement not covered" >      startTime = hrtime.bigint();</span>
      const earliestOffsets = <span class="cstat-no" title="statement not covered" >await this.#listOffsets(topicPartitionOffsetsEarliest, options);</span>
<span class="cstat-no" title="statement not covered" >      endTime = hrtime.bigint();</span>
<span class="cstat-no" title="statement not covered" >      timeTaken = Number(endTime - startTime) / 1e6; </span>// Convert nanoseconds to milliseconds
&nbsp;
      // Adjust timeout for the next request
<span class="cstat-no" title="statement not covered" >      options.timeout -= timeTaken;</span>
<span class="cstat-no" title="statement not covered" >      if (options.timeout &lt;= 0) {</span>
<span class="cstat-no" title="statement not covered" >        throw new error.KafkaJSError("Timeout exceeded while fetching earliest offsets.", { code: error.ErrorCodes.ERR__TIMED_OUT });</span>
      }
&nbsp;
      const combinedResults = <span class="cstat-no" title="statement not covered" >partitionIds.map(<span class="fstat-no" title="function not covered" >pa</span>rtitionId =&gt; {</span>
        const latest = <span class="cstat-no" title="statement not covered" >latestOffsets.find(<span class="fstat-no" title="function not covered" >of</span>fset =&gt; <span class="cstat-no" title="statement not covered" >offset.partition === partitionId)</span>;</span>
        const earliest = <span class="cstat-no" title="statement not covered" >earliestOffsets.find(<span class="fstat-no" title="function not covered" >of</span>fset =&gt; <span class="cstat-no" title="statement not covered" >offset.partition === partitionId)</span>;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >        return {</span>
          partition: partitionId,
...
&nbsp;
<span class="cstat-no" title="statement not covered" >      return combinedResults;</span>
    } catch (err) {
<span class="cstat-no" title="statement not covered" >      throw createKafkaJsErrorFromLibRdKafkaError(err);</span>
    }
  }
...
<span class="cstat-no" title="statement not covered" >    return new Promise(<span class="fstat-no" title="function not covered" >(r</span>esolve, reject) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      this.#internalClient.listOffsets(partitionOffsets, options, <span class="fstat-no" title="function not covered" >(e</span>rr, offsets) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        if (err) {</span>
<span class="cstat-no" title="statement not covered" >          reject(createKafkaJsErrorFromLibRdKafkaError(err));</span>
        } else {
<span class="cstat-no" title="statement not covered" >          resolve(offsets);</span>
        }
      });
    });
  }
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  as</span>ync fetchTopicOffsetsByTimestamp(topic, timestamp, options = <span class="branch-0 cbranch-no" title="branch not covered" >{})</span> {
<span class="cstat-no" title="statement not covered" >    if (this.#state !== AdminState.CONNECTED) {</span>
<span class="cstat-no" title="statement not covered" >      throw new error.KafkaJSError("Admin client is not connected.", { code: error.ErrorCodes.ERR__STATE });</span>
    }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >      topic.partitions.map(<span class="fstat-no" title="function not covered" >pa</span>rtition =&gt; <span class="cstat-no" title="statement not covered" >partition.partitionId)</span></span>
    ...
    let topicPartitionOffset = <span class="cstat-no" title="statement not covered" >[];</span>
<span class="cstat-no" title="statement not covered" >    if (typeof timestamp === 'undefined') {</span>
<span class="cstat-no" title="statement not covered" >      topicPartitionOffset = partitionIds.map(<span class="fstat-no" title="function not covered" >pa</span>rtitionId =&gt; (<span class="cstat-no" title="statement not covered" >{</span></span>
        topic,
        partition: partitionId,
        offset: OffsetSpec.LATEST
      }));
...
<span class="cstat-no" title="statement not covered" >      startTime = hrtime.bigint();</span>
      const offsetsByTimeStamp = <span class="cstat-no" title="statement not covered" >await this.#listOffsets(topicPartitionOffset, options);</span>
<span class="cstat-no" title="statement not covered" >      endTime = hrtime.bigint();</span>
<span class="cstat-no" title="statement not covered" >      timeTaken = Number(endTime - startTime) / 1e6; </span>// Convert nanoseconds to milliseconds
&nbsp;
      // Adjust timeout for the next request
<span class="cstat-no" title="statement not covered" >      options.timeout -= timeTaken;</span>
<span class="cstat-no" title="statement not covered" >      if (options.timeout &lt;= 0) {</span>
<span class="cstat-no" title="statement not covered" >        throw new error.KafkaJSError("Timeout exceeded while fetching offsets.", { code: error.ErrorCodes.ERR__TIMED_OUT });</span>
      }
&nbsp;
<span class="cstat-no" title="statement not covered" >      if (typeof timestamp === 'undefined') {</span>
        // Return result from offsetsByTimestamp if timestamp is undefined
<span class="cstat-no" title="statement not covered" >        return offsetsByTimeStamp.map(<span class="fstat-no" title="function not covered" >of</span>fset =&gt; (<span class="cstat-no" title="statement not covered" >{</span></span>
...
&nbsp;
        const combinedResults = <span class="cstat-no" title="statement not covered" >partitionIds.map(<span class="fstat-no" title="function not covered" >pa</span>rtitionId =&gt; {</span>
          const latest = <span class="cstat-no" title="statement not covered" >latestOffsets.find(<span class="fstat-no" title="function not covered" >of</span>fset =&gt; <span class="cstat-no" title="statement not covered" >offset.partition === partitionId)</span>;</span>
          const timestampOffset = <span class="cstat-no" title="statement not covered" >offsetsByTimeStamp.find(<span class="fstat-no" title="function not covered" >of</span>fset =&gt; <span class="cstat-no" title="statement not covered" >offset.partition === partitionId)</span>;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >          if (timestampOffset.offset === -1) {</span>
<span class="cstat-no" title="statement not covered" >            return {</span>
              partition: partitionId,
              offset: latest.offset.toString(),
            };
