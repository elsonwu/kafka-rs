...
use super::{entities::*, errors::*, repositories::*, value_objects::*};
...
impl MessageService {
    ...
    pub async fn send_message(&self, topic_name: TopicName, message: Message) -> Result<Offset> {
        ...
        self.topic_repo
            .save(&topic)
            .await
            .map_err(|e| DomainError::InvalidMessage(format!("Failed to save topic: {}", e)))?;

        info!(
            "Message stored at offset {} in topic {}",
            offset, topic_name
        );
        Ok(offset)
    }
    ...
    pub async fn create_topic(&self, topic_name: TopicName) -> Result<()> {
        ...
        self.topic_repo
            .save(&topic)
            .await
            .map_err(|e| DomainError::InvalidMessage(format!("Failed to create topic: {}", e)))?;
